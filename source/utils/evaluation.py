...
def get_scores_generation(eval_outputs, path, data, hit_thres=0):
    df = pd.concat([pd.DataFrame(d) for d in eval_outputs])
    ...
    Morgan_sims = []
    if "single" in data:
        for pred, label in tqdm.tqdm(zip(df['pred'].values.tolist(), df['label'].values.tolist())):
            try:
